...
## Some Deviations from the GFM Spec

### Tables
A row in the table must have leading and trailing '|' characters or the cells in that row 
won't be recognised. Pipes without any content in between(i.e '||') isn't allowed, 
It must have some content if it's just whitespace